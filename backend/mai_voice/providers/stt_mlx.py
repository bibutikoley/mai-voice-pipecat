...
process (`make audio-server`), which binds to 127.0.0.1 only. The container
reaches it via `host.docker.internal`; phones on the LAN cannot.
"""
...
def create(settings: Settings) -> OpenAISTTService:
    return OpenAISTTService(
        api_key=_FAKE_KEY,
        base_url=f"{settings.mlx_audio_base_url.rstrip('/')}/v1",
